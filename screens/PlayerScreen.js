...
const PlayerScreen = ({ route }) => {
  ...
  useEffect(() => {
    const fetchPlayersData = async () => {
      const promises = team.players.map(async (player) => {
        try {
          const response = await axios.get(`https://api.sportmonks.com/v3/football/players/${player.player_id}?include=country;position`, {headers:{Authorization:'TL6Gh8pelPNE0dfFrjTAEc6UD3eAdgnq1PuqigxjirGAk7XCyEJkvszFiMPx'}});
  ...
  const renderItem = ({ item }) => (
    <TouchableOpacity onPress={() => handlePlayerPress(item)} style={{ width: 160, height: 75 }}>
